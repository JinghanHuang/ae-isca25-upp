...
def parse_usps_filtered_ratios(filepath):
    filtered_ratios = []
    current_query = {}
...
def parse_baseline_filtered_ratios(filepath, all_usps_tables):
    filtered_ratios = []
    with open(filepath, 'r') as f:
        content = f.read()
    ...
    for i in range(1, 23):
        ...
        if i - 1 < len(all_usps_tables):
            for table in all_usps_tables[i - 1].keys():
...
def save_combined_latency_table(filename, base_s, base_c, usps_s, usps_c, baseline_ratios, usps_ratios):
    with open(filename, 'w') as f:
        ...
        for i in range(len(base_s)):
            ...
            total_usps = usps_s[i] + usps_c[i]
            f.write(f'| {i+1:<6} | {base_s[i]:<28.6f} | {usps_s[i]:<29.6f} | {base_c[i]:<27.6f} | {usps_c[i]:<27.6f} | {total_base:<19.6f} | {total_usps:<19.6f} | {baseline_ratios[i]:<24.4f} | {usps_ratios[i]:<20.4f} |\n')

def save_energy_stats(filename, base_s, base_c, usps_s, usps_c, stats):
    with open(filename, 'w') as f:
        ...
        for i in range(len(base_s)):
            ...
            usps_key = f"{query}-usps"
            base_latency = base_s[i] + base_c[i]
            usps_latency = usps_s[i] + usps_c[i]

            b = stats.get(base_key)
            u = stats.get(usps_key)
            if not b or not u:
                continue

            cpu_energy_b = (b['cpu_power_diff'] / b['cpu_power_lines']) * base_latency / 1000 if b['cpu_power_lines'] else 0.0
            cpu_energy_u = (u['cpu_power_diff'] / u['cpu_power_lines']) * usps_latency / 1000 if u['cpu_power_lines'] else 0.0

            smart_energy_b = 0
            smart_energy_u = u['smartssd_power'] * usps_latency / 1000

            other_energy_b = ((b['system_power'] - 272) * base_latency - cpu_energy_b - smart_energy_b) / 1000
            other_energy_u = ((u['system_power'] - 272) * usps_latency - cpu_energy_u - smart_energy_u) / 1000
            ...
            cpu_util_u = u['cpu_util'] * usps_latency
...
def main():
    ...
    usps_path = 'runningtime_usps.txt'
    ...
    usps_storage, usps_compute = parse_latencies(usps_path)

    # Parse filtered ratio results about figure 6
    usps_table_ratios = parse_usps_filtered_ratios(usps_path)
    usps_ratios = calculate_filtered_ratio(usps_table_ratios)
    baseline_ratios = parse_baseline_filtered_ratios(filter_log_path, usps_table_ratios)

    # Parse results about figure 7
    stats = read_compiled_stats(stats_path)

    save_combined_latency_table('figure6_data.txt', base_storage, base_compute, usps_storage, usps_compute, baseline_ratios, usps_ratios)
    save_energy_stats('figure7_data.txt', base_storage, base_compute, usps_storage, usps_compute, stats)
